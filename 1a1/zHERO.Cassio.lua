	...
	function Cassiopeia:__init()
		...
		Cass = MenuElement({type = MENU, id = "Cass", name = "Weedle | Cassiopeia"})
		self:Menu()	
		...
		print("Weedle | Cassiopeia | Loaded")
	end
	...
	function Cassiopeia:Menu()
		...
		Cass.h:MenuElement({id = "Q", name = "Use Q", value = true})
		Cass.h:MenuElement({id = "W", name = "Use W", value = true})
		Cass.h:MenuElement({id = "E", name = "Use E", value = true})		
		...
		Cass.m:MenuElement({name = " ", drop = {"Combo, Harass [%]"}})
		...
		Cass:MenuElement({type = MENU, id = "t", name = "Tear Stacker"})
		Cass.t:MenuElement({id = "ON", name = "Tear Stacker Toggle Key", key = 83, toggle = true})
		Cass.t:MenuElement({id = "Mana", name = "Min % Mana", value = 10, min = 0, max = 100, step = 1})
		Cass:MenuElement({type = MENU, id = "a", name = "Activator"})
		--Cass.a:MenuElement({type = MENU, id = "Ignite", name = "Ignite"})
		--Cass.a.Ignite:MenuElement({id = "ON", name = "Enable in Combo", value = true})
		--Cass.a.Ignite:MenuElement({id = "HP", name = "Min Target HP %", value = 80, min = 0, max = 100, step = 1})
		Cass.a:MenuElement({type = MENU, id = "Potion", name = "Corrupting Potion"})
		Cass.a.Potion:MenuElement({id = "ON", name = "Enable in Combo", value = true})
		...
		Cass.a:MenuElement({type = SPACE, name = "PM me if I missed an Item"})
	...
	function Cassiopeia:Edmg(unit)
		return CalcMagicalDamage(myHero, unit, 48 + 4 * myHero.levelData.lvl + 0.1 * myHero.ap)
	end

	function Cassiopeia:PEdmg(unit)
		return CalcMagicalDamage(myHero, unit, 58 + 4 * myHero.levelData.lvl + 30 * myHero:GetSpellData(_E).level + 0.45 * myHero.ap )
	end
	...
	function Cassiopeia:Activator(Mode)
		...
		if Mode == "Combo" then
			...
			if Cass.a.Potion.ON:Value() then
			local Potion = GetItemSlot(myHero, 2033)
				if Potion >= 1 and Ready(Potion) and NoPotion() then 
					Control.CastSpell(ItemHotKey[Potion])
				end
			end
		end
	end
	...
	function Cassiopeia:Combo()
		...
		if Cass.c.W:Value() and Ready(_W) and (((Ready(_R) and RTarget) or RValue == false) or ((QWReady == false and (Game.Timer() + QData.cd) - QData.castTime > 0.80) or QValue == false)) and (HasPoison(target) == false or QWReady == false and Ready(_E) == false) and myHero.mana/myHero.maxMana > Cass.m.W:Value()/100 then 
		...
		if Cass.c.R:Value() and Ready(_R) and (HasPoison(target) or Panic) and ((WCheck == false or (WCheck and (Game.Timer() + WData.cd) - WData.castTime > 2)) or WValue == false) then
			if Panic then
				if Dist < RRange and myHero.mana/myHero.maxMana > Cass.m.R:Value()/100 and self:PEdmg(target) < target.health then
					if RTarget then
			...
			if RTarget and ShouldCast and RTarget.health/RTarget.maxHealth > 0.2 then 
				if Dist < RRange then
				local RPos = GetPred(RTarget, 1500, 0.25 + Game.Latency()/1000)
					Control.CastSpell(HK_R, RPos)
				end
			end
		end
		if QValue and Ready(_Q) and (((Ready(_W) == false or WValue == false) or HasPoison(target) == false) or (Ready(_R) == false or RValue == false)) and myHero.mana/myHero.maxMana > Cass.m.Q:Value()/100 then 
			if Dist < QRange then 
	...
	function Cassiopeia:Harass()
		...
		local QData = myHero:GetSpellData(_Q)
		if Cass.h.W:Value() and Ready(_W) and ((Ready(_Q) == false and (Game.Timer() + QData.cd) - QData.castTime > 0.80) or QValue == false) and HasPoison(target) == false and myHero.mana/myHero.maxMana > Cass.m.W:Value()/100 then 
			if Dist < MaxWRange and Dist > MinWRange then
			local Pos = GetPred(target, 1500, 0.25 + Game.Latency()/1000)
				if GetDistanceSqr(Pos, myHero.pos) < MaxWRange then
		...
		if Cass.h.E:Value() and Ready(_E) and ((HasPoison(target) or Dist < MinWRange) or self:Edmg(target) * 2 --[[Lets be honest here it's the combo sometimes u just need it xD]] > target.health) and myHero.mana/myHero.maxMana > Cass.m.E:Value()/100 then 
			if Dist < ERange then
				Control.CastSpell(HK_E, target)
			end
		end
	...
	function Cassiopeia:Draw()
		if myHero.dead == false and Cass.d.ON:Value() then
			...
			if Cass.t.ON:Value() and Tear then
				Draw.Text("Tear Stacker ON", 20, textPos.x - 80, textPos.y + 60, Draw.Color(255, 000, 000, 200))
			end
